...
class UserTimelineDataSourceFactory(
    private val query: String,
    private val client: TwitterClient
) : DataSource.Factory<Long, Tweet>() {
    val sourceLiveData = MutableLiveData<UserTimelineDataSource>()
    override fun create(): DataSource<Long, Tweet> {
        val source = UserTimelineDataSource(query, client)
        sourceLiveData.postValue(source)
        return source
    }
}

class UserTimelineDataSource(
    private val query: String,
    private val client: TwitterClient
