...
interface TwitterLocalDataSource {
    ...
    suspend fun getTweetsFromList(listId: Long): Result<List<Tweet>>
}
...
class TwitterDataSourceImpl internal constructor(
    ...
    override suspend fun getTweetsFromList(listId: Long) = withContext(ioDispatcher) {
        return@withContext try {
            Success(tweetDao.getTweetsByListId(listId))
        } catch (e: Exception) {
            Error(e)
        }
    }
